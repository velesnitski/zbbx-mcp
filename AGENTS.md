...
`fetch_all_data()` is the central data-fetch function used by all report tools. It fetches dashboards and hosts in parallel (phase 1), then fires ~13 `item.get` calls simultaneously (phase 2) for CPU, memory, traffic, VPN check items, macros, and templates. Missing traffic data is filled with a fallback name-based search (phase 3). Results are assembled into `ServerRow` dataclasses and sorted (dashboard hosts first).
