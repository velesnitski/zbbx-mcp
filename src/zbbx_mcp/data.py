...
__all__ = [
    ...
    "fetch_enabled_hosts", "fetch_traffic_map", "fetch_cpu_map", "fetch_vpn_status", "fetch_host_dashboards",
...
STATUS_ENABLED = "0"
# VPN health check item keys — configurable per deployment
KEY_VPN_PRIMARY = os.environ.get("ZABBIX_VPN_CHECK_KEY", "")
KEY_VPN_SECONDARY = os.environ.get("ZABBIX_VPN2_CHECK_KEY", "")
KEY_VPN_TERTIARY = os.environ.get("ZABBIX_VPN3_CHECK_KEY", "")
...
def extract_country(hostname: str) -> str:
    ...
    Handles: srv-nl0105 → NL, srv-nl01-lite → IN, srv-us01-lite → US
    Normalizes UK → GB.
    """
...
async def fetch_vpn_status(client: ZabbixClient, hostids: list[str]) -> dict[str, int]:
    ...
    if not hostids:
        return {}
    keys = [k for k in (KEY_VPN_PRIMARY, KEY_VPN_SECONDARY, KEY_VPN_TERTIARY) if k]
    if not keys:
        return {}
...
class ServerRow:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "VPN Primary": self.vpn_primary,
            "VPN Secondary": self.vpn_secondary,
            "VPN Tertiary": self.vpn_tertiary,
            "Agent": self.agent,
...
async def fetch_all_data(
    ...
    results = await asyncio.gather(
        ...
        _item_call(KEY_VPN_PRIMARY),
        _item_call(KEY_VPN_SECONDARY),
        _item_call(KEY_VPN_TERTIARY),
