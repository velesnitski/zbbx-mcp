...
__all__ = ["compute_host_uptime", "coverage_note", "retention_too_short"]
...
def compute_host_uptime(
    ...
    means the hour was mostly up). ``host_has_traffic``: did the host move
    real traffic over the window (the deprecated-check false-down guard,
    task 169).
    ...
    - missing hour, host had traffic → UP (host was alive, the check just
      wasn't recording — kills the false-down class);
    ...
    for h in range(first_h, now_h + 1):
        ...
        elif host_has_traffic:
            up += 1  # alive per traffic, check silent
