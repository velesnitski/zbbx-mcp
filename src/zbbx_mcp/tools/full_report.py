        ...
        async def generate_full_report(
            ...
            try:
                ...
                (cpu_items, load_items, mem_items, conn_items,
                 ...
                 version_items, vpn1_items, cost_macros, graphs_raw) = results
                ...
                vpn1_map = _build_map(vpn1_items, lambda v: int(float(v)))
                ...
                for hid, h in host_map.items():
                    ...
                    vpn1 = vpn1_map.get(hid)
                    vpn1_status = ""
                    if vpn1 is not None:
                        vpn1_status = "OK" if vpn1 == 1 else "DOWN"
                ...
                for idx, r in enumerate(rows, 2):
                    ...
                    # VPN Primary status coloring
                    vpn1_val = r.get("VPN Primary")
                    if vpn1_val:
                        vpn1_cell = ws1.cell(row=idx, column=vpn1_col)
                        if vpn1_val == "DOWN":
                            vpn1_cell.fill = red_fill
                        elif vpn1_val == "OK":
                            vpn1_cell.fill = green_fill
