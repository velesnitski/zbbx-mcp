...
{_kpi_card("VPN Health", f"{vpn_down} DOWN" if vpn_down else "All OK", f"{total - vpn_down} healthy" if vpn_down else f"{sum(1 for r in rows if r.get('VPN Primary')=='OK')} monitored", "red" if vpn_down else "green")}
</div>
"""]
                ...
                for cc in country_servers:
                    ...
                    cc_vpn_down = sum(1 for r in rows if r.get("Country") == cc and r.get("VPN Primary") == "DOWN")
                    if cc_vpn_down > 0:
                        risks.append(f"{cc_vpn_down} VPN DOWN")
                    if risks:
                ...
                actions = []
                if vpn_down:
                    actions.append(f'<span class="badge badge-red">{vpn_down} VPN DOWN</span>')
                if high_cpu:
