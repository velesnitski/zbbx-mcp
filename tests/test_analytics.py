...
class TestProviderDetection:
    ...
    def test_aws_16(self):
        assert detect_provider("16.16.190.129") == "AWS"

    def test_digitalocean(self):
        assert detect_provider("134.209.100.1") == "DigitalOcean"
    ...
    def test_prefix_length_wins(self):
        ...
        assert detect_provider("34.64.1.1") == "Google Cloud"
        assert detect_provider("54.36.1.1") == "OVH"
        assert detect_provider("52.224.1.1") == "Azure"
...
class TestProductHiding:
    ...
    def test_ceo_report_two_step_filter(self, monkeypatch):
        """Simulate the actual CEO report flow: vpn_hosts then fleet composition.

        This catches the real bug where fleet composition iterated `hosts`
        instead of `vpn_hosts`, leaking hidden products into the report.
        """
        monkeypatch.setenv("ZABBIX_HIDE_PRODUCTS", "Legacy")
        ...
        all_hosts = [
            {"hostid": "1", "host": "srv-de01", "groups": [{"name": "ActiveVPN"}]},
            {"hostid": "2", "host": "srv-de02", "groups": [{"name": "ActiveVPN"}]},
            {"hostid": "3", "host": "srv-nl01", "groups": [{"name": "ActiveVPN"}]},
        ...
        # Step 1: Build vpn_hosts (same as CEO report line 144-146)
        vpn_hosts = [
            h for h in all_hosts
            if not _data.is_hidden_product(classify_host(h.get("groups", []))[0])
            and classify_host(h.get("groups", []))[0] not in _NON_VPN
        ]

        assert len(vpn_hosts) == 3, f"Expected 3 VPN hosts, got {len(vpn_hosts)}"

        # Step 2: Build fleet composition from vpn_hosts (NOT all_hosts!)
        product_counts: dict[str, int] = {}
        for h in vpn_hosts:  # Must be vpn_hosts, not all_hosts
            prod, _ = classify_host(h.get("groups", []))
            if prod:
                product_counts[prod] = product_counts.get(prod, 0) + 1

        assert "ActiveVPN" in product_counts
        assert product_counts["ActiveVPN"] == 3
        ...
        total_from_header = len(vpn_hosts)
