...
| [083](083-mcp-1281-and-click-cve.md) | `mcp` 1.28.1 + `click` — second CVE round, and a cap correction | Auditing after ADR 082 found two more: CVE-2026-59950 (mcp WebSocket transport, not reachable via FastMCP so we're not exploitable, fixed 1.28.1) and PYSEC-2026-2132 (click.edit injection, transitive via uvicorn, fixed 8.3.3). And ADR 082's own `<1.28.0` cap blocked the 1.28.1 fix within the hour — the anti-pattern it warned about. Raised to `>=1.28.1,<2.0.0` (major-bound; graceful degradation + the subprocess test are the net), click lockfile-only to 8.4.2. pip-audit now clean |
| [084](084-server-test-stdio-race.md) | Fix a stdio-shutdown race in the server subprocess test | After the mcp 1.28.1 bump, CI went red on `test_server` — but only in CI. The harness batch-wrote the JSON-RPC messages and closed stdin immediately (`subprocess.run(input=...)`), racing the hardened SDK's EOF-triggered stdio teardown against the pending `tools/list` handler: shutdown won on the slow runner, the handler won locally. Rewrote it to hold stdin open until every request has its response, then close. Test-sync fix, server was correct |
| [085](085-get-users-output-field-and-guard.md) | `get_users` -32602 (removed output field) + output-field guard | `user.get` output requested `type` (removed 5.2 → `roleid`) + `rows_per_page`, so get_users was dead on every call. No guard covered top-level `output` fields, and its params were built in a *variable* (invisible to the inline-only scanners). Fixed output + resolve role names via `role.get`; new output-field guard resolves variable-built params. +7 tests, 759 → 766 |
| [086](086-iso-daily-keys.md) | ISO-sortable daily trend keys (month-boundary sort bug) | Daily keys were `"%b %d"`, which sorts lexically (`Jul < Jun`), so any window crossing a month boundary scrambled `sorted(daily)` — silently flipping verdicts in `get_traffic_drop_timeline` (missed active blocks), `detect_regional_anomalies`, `get_executive_dashboard`, plus geo/CEO trend direction. Fixed at source with `"%Y-%m-%d"` keys + a `day_label` render helper. +4 tests, 766 → 770 |
| [087](087-traffic-unit-reconciliation.md) | One traffic-unit conversion (`get_peak_analysis` 8×, bytes divisor 64×) | `get_peak_analysis` hardcoded `*8/1e6`, so on the default bits/s config it reported **8× the true Mbps** and disagreed with every other tool. The bytes-mode divisor was `8_000_000` but should be `125_000` (64× off). Routed peak through the shared `TRAFFIC_DIVISOR` and fixed the divisor — one conversion, correct in both modes. +4 tests, 770 → 774 |
| [088](088-silent-degradation-cluster.md) | Silent-degradation cluster — removed/renamed API fields | Four fields removed/renamed in modern Zabbix that this instance *silently ignores* (blank column, not a crash): `host.get available` → `active_available` (6.0); `discoveryrule.get lastclock` (never existed → permanent 1970); `item.get selectHosts ["groups"]` (can't nest → resolve via separate host.get); `maintenance.get selectGroups` → added to the client shim. Output-field guard extended. +5 tests, 774 → 779 |
| [089](089-test-exclusion-seam.md) | A shared `exclude_test` seam for fleet verdicts | ADR 080 wired 6 tools; most fleet-verdict tools still counted test boxes. Added `exclude_test` to the shared `fetch_enabled_hosts` (forces groups, filters via `is_test_host`, cache-keyed) instead of a risky 20-tool sweep — a blanket sweep would wrongly hide test hosts from `search_hosts`/maps/CRUD. Wired `detect_traffic_anomalies` (named-note) + CEO/service/expansion reports. Long tail deferred deliberately. +4 tests, 779 → 783 |
| [090](090-check-flap-matrix.md) | `detect_check_flaps` — minute-level flap matrix with a noise/real split | A live flap audit proved: same-minute dips on distant hosts = prober noise (subtract first); TEST-class checks flap several times more than prod (weight 0); chronically degraded hosts (prod check failing most hours, 1–2 min at a time) fire **zero** triggers. New bounded tool classifies every flap-minute (fleet noise → host event → test noise → residual rate) and names the chronic-but-triggerless hosts. Tool count 163 → 164. +14 tests, 783 → 797 |
