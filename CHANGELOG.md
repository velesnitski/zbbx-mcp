...
- `ZABBIX_VPN_CHECK_KEY`, `ZABBIX_VPN2_CHECK_KEY`, `ZABBIX_VPN3_CHECK_KEY` — configurable VPN health check item keys
...
- Country deep dives with auto-detection (dead, blocked, growth, no VPN checks)

### Changed
- VPN check keys fully configurable via env vars (no hardcoded item keys)
