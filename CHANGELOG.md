...
- `product` filter on `detect_regional_anomalies`
...
- Token optimization for 6 tools (tasks 63–68):
  - `get_service_uptime_report`: `only_problems=True` default, `exclude_product`, `max_results`
...
### Added
- `get_service_health_matrix`: per-country service health status matrix with recommendations
- `get_traffic_drop_timeline`: when blocks started per country from daily trend data
...
- `extract_country` regex fixed for Lite hostnames (`srv-nl03` → IN)
...
- **Traffic anomaly detection**: `detect_traffic_anomalies` (peer comparison), `detect_traffic_drops` (trend-based regional outageing detection), `get_traffic_report`
