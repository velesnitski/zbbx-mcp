...
| `dead_protocols.py` | 1 | `detect_dead_protocols` — a protocol dead behind an UP-if-any host; groups by CHECK so a fleet-wide outage reads as one row |
...
| `traffic_totals.py` | 1 | `get_traffic_totals` — one total for a fleet slice, carrier NIC per host, uncovered hosts named rather than summed as zero |
